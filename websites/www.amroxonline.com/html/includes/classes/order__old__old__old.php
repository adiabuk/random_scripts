  ...
  class order {
    ...
    function cart() {
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
		...
		//////////////////////Start Code changes/////////////////////////////
		$product_query = tep_db_query("select products_id, products_price, products_tax_class_id, products_weight from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products[$i]['id'] . "'");
		...
		$products_price =  $products[$i]['price'] ;//$products[$i]['quantity']); 
    	$products_attr_price = $cart->attributes_price($products[$i]['id']);
		 
		if($_SESSION["prod_".$products[$i]['id']]['id']==$products[$i]['id']){
			if($products_attr_price>0.01){
				$act_product_price  = ($products_price + $products_attr_price) * $_SESSION["prod_".$products[$i]['id']]['price'];
			}
			else{
				$act_product_price  = ($products_price * $_SESSION["prod_".$products[$i]['id']]['price']);
			}			  
		}
		else{
		...
		///$act_product_price = $products[$i]['price'] + $cart->attributes_price($products[$i]['id']);
		...
		$shown_price = $currencies->calculate_price($this->products[$index]['final_price'], 0, $this->products[$index]['qty']);
