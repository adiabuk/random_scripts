...
$orders_products_query = tep_db_query("select a.orders_products_id, a.products_id, a.products_name, a.products_model, a.products_price, a.products_tax, a.products_quantity, a.final_price, b.products_price as `amt`, b.products_tax_class_id from " . TABLE_ORDERS_PRODUCTS . " a, " . TABLE_PRODUCTS . " b where a.products_id = b.products_id and a.orders_id = '" . (int)$order_id . "'");
      ...
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
        ...
        $rp_product_query = tep_db_query('SELECT billingPeriod, billingFrequency, totalBillingCycles, profileStartDate, trialBillingPeriod, trialBillingFrequency, trialTotalBillingCycles, trialAmt, initAmt, failedInitAmtAction FROM ' . TABLE_RP_PAYPAL_PRODUCT_PROFILE . ' WHERE products_id=' . (int)$orders_products['orders_products_id']);
    ...
    function cart() {
      ...
      $products = $cart->get_products(false);
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                        ...
                                        #'final_price' => $products[$i]['final_price']/* + $cart->attributes_price($products[$i]['id'])*/,
                                        'weight' => $products[$i]['weight'],
                                        #'id' => $products[$i]['id']);
                                        'id' => $products[$i]['id'],
                                        'tax_class_id' => $products[$i]['products_tax_class_id']);
        // rp add rp array to the product - if it is a rp product
        $rp_product_query = tep_db_query('SELECT billingPeriod, billingFrequency, totalBillingCycles, profileStartDate, trialBillingPeriod, trialBillingFrequency, trialTotalBillingCycles, trialAmt, initAmt, failedInitAmtAction FROM ' . TABLE_RP_PAYPAL_PRODUCT_PROFILE . ' WHERE products_id=' . (int)$products[$i]['id']);
        if($rp_product = tep_db_fetch_array($rp_product_query)){
            $this->products[$index]['rp'] = $rp_product;
            $this->products[$index]['rp']['amt'] = $this->products[$index]['price'];
            if(strlen($rp_product['trialBillingPeriod'])>0){
                $this->products[$index]['price'] = $rp_product['trialAmt'];
                $this->products[$index]['final_price'] = $rp_product['trialAmt'] + $cart->attributes_price($products[$i]['id']);
            }else{
                $this->products[$index]['final_price'] = $products[$i]['price'] + $cart->attributes_price($products[$i]['id']);
            }
        }else{
            $this->products[$index]['final_price'] = $products[$i]['price'] + $cart->attributes_price($products[$i]['id']);
            $this->products[$index]['rp'] = array();
