...
#($name, $values, $default = '', $parameters = '', $required = false) {
  function tep_draw_radio_menu($name, $values, $default = '', $parameters = '', $required = false) {
    ...
    for ($i=0, $n=sizeof($values); $i<$n; $i++) {
      ...
      $field .= '<input type="radio" name="' . $name . '" '.$parameters.' value="' . $value . '"';
#      $field .= '<input type="radio" name="' . $name . '" onClick="showPrice(this.form);" value="' . $value . '"';
#onChange='showPrice(this.value); '
        if ($i == 0) $field .= ' checked';



      $field .= '><span id="option'.$value.'"> ' . tep_output_string($values[$i]['text'], array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '</span><br>';

    }

    $field .= '</td></tr></table>';
####Added by adiab 01/04/10##########
$string=$field;
###$patterns[] = '/\(([\+-])£([0-9\.]+)\)/';
###$replacements[] = '<font color=white>($1£$2)</sfont>';

$patterns[] = '/\([\*]£([0-9\.]+)\)/';
$replacements[] = '<font color=white>(*£$1)</font>';

$field=preg_replace($patterns, $replacements, $string);
######################################
