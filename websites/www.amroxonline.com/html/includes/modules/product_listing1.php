...
$category_query = tep_db_query("select cd.categories_name, cd.categories_description, c.categories_image from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . $current_category_id . "' and cd.categories_id = '" . $current_category_id . "' and cd.language_id = '" . $languages_id . "'");

    $category = tep_db_fetch_array($category_query);

	$cat_desc = $category['categories_description'];
	$cat_desc1 = substr($cat_desc , 0 , strpos($cat_desc , '<div class="bodytexb">'));
	$cat_desc2 = substr($cat_desc , strpos($cat_desc , '<div class="bodytexb">'));
	$cat_desc3 = substr($cat_desc2 , strpos($cat_desc , '</div>'));
	$cat_desc2 = substr($cat_desc2 , 0 , strpos($cat_desc , '</div>')).'</div>';
?>

<table cellpadding="0" cellspacing="0" class="main">
		<tr><td><?php echo $cat_desc1.$cat_desc3 ?></td></tr>
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_PRICE':
           ...
           $p_price = $lc_text = '<s>' .  $currencies->display_price($listing['products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</s><br><span class="productSpecialPrice">' . $currencies->display_price($listing['specials_new_products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</span>';
            } else {
...
$product_query = tep_db_query("select products_description, products_description2, products_id from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$listing['products_id'] . "' and language_id = '" . (int)$languages_id . "'");
...
/*		$list_box_contents[$cur_row][] = array('align' => $lc_align,
 ...
 $col_max_product_in_row = MAX_PRODUCT_IN_ROW2; 
 $info_box_contents[$my_row][$my_col] = array('align' => 'center',
	...
	<table cellpadding="0" cellspacing="0" border="0">
		...
		<tr><td>
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td class="pic3_padd">'.tep_draw_prod_pic_top().''.$p_pic.''.tep_draw_prod_pic_bottom().'</td>
					<td style="width:100%;">
						<table cellpadding="0" cellspacing="0" border="0">
							<tr><td class="desc desc3_padd">'.$p_desc2.'</td></tr>
						</table>
					</td>
					
				</tr>
				<tr><td colspan="2" class="listing2_padd">'.$p_listing.'</td></tr>
				<tr>
					<td class="price3_padd">'.$p_price.'</td>
					<td class="button3_padd button_marg">'.$p_details.'  '.$p_buy_now .'</td>
				</tr>
			</table>
		</td></tr>
	</table>										   
    ...
    $my_col ++;
    if ($my_col > $col_max_product_in_row) {	
      $my_col = 0;
 	$my_row ++;
      }
    }
...
?>
<table cellpadding="0" cellspacing="0" class="main">
		<tr><td><?php echo $cat_desc2 ?></td></tr>
                <tr><td><?php echo "<br>" ?></td></tr>
</table>
...
?>
				...
				<table cellpadding="0" cellspacing="0" class="main">
				<tr><td><?php echo $category['categories_description'] ?></td></tr>
					<tr><td><?php echo "<br>" ?></td></tr>
