...
//---PayPal WPP Modification END ---//
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'update_order':
        ...
        // can not change to review FMF status
        if($status == 5){
            tep_redirect(tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action')) . 'action=edit'));
        }
        if ( ($check_status['orders_status'] != $status) || tep_not_null($comments)) {
            // check to see if this is a FMF order status edit
            $sql = 'SELECT orders_status FROM ' . TABLE_ORDERS . ' WHERE orders_id = ' . (int)$oID;
            if($rs = tep_db_query($sql)){
                $row = tep_db_fetch_array($rs);
                if($row['orders_status'] == 5){  // status 5 hardcoded, should not be hardcoded
                    // fmf require
                    require_once(DIR_WS_CLASSES . 'paypal_fmf.php');
                    $fmf_paypal = new paypal_fmf($oID);
                    if($status<4){
                        $return = $fmf_paypal->set_status('accept');
                    }elseif($status==6){
                        $return = $fmf_paypal->set_status('deny');
                        $comments .= " This order has been denied.";
                    }
                }if($row['orders_status'] == 6){
                    // already been denied no change available
                    tep_redirect(tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action')) . 'action=edit'));
                }
            }
  ...
  if (($action == 'edit') && (isset($HTTP_GET_VARS['oID']) or isset($HTTP_GET_VARS['oRpId']) )) {
    // rp profile id search
    if(isset($HTTP_GET_VARS['oRpId'])){
        $rp_order_query = tep_db_query('SELECT orders_id FROM ' . TABLE_ORDERS_PRODUCTS . ' WHERE rp_profile_id = \''.$HTTP_GET_VARS['oRpId'].'\'');
        if($rp_order = tep_db_fetch_array($rp_order_query)){
            $HTTP_GET_VARS['oID'] = $rp_order['orders_id'];
        }
    }
    $oID = tep_db_prepare_input($HTTP_GET_VARS['oID']);
...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      ...
      if(array_key_exists('rp_paypal',$order->products[$i])){
        if(is_array($order->products[$i]['rp_paypal'])){
            if(array_key_exists('PROFILEID',$order->products[$i]['rp_paypal'])){
                if(strlen($order->products[$i]['rp_paypal']['PROFILEID'])>0){
                    print '&nbsp;&nbsp;<a href="'.tep_href_link(FILENAME_RP, 'rpId='.$order->products[$i]['rp_paypal']['PROFILEID']) .'">[Recurring Profile Information]</a>';
                }
            }
        }
      }
          ...
          <tr>
            ...
            <td align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr><?php echo tep_draw_form('orders', FILENAME_ORDERS, '', 'get'); ?>
                <td class="smallText" align="right"><?php echo HEADING_TITLE_SEARCH . ' ' . tep_draw_input_field('oID', '', 'size="12"') . tep_draw_hidden_field('action', 'edit'); ?>&nbsp;or&nbsp;Recurring Profile Id <?php print tep_draw_input_field('oRpId', '', 'size="14"') . tep_draw_hidden_field('action', 'edit');?><input type="submit" value="search" name="search"></td>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_ORDERS_TYPE; ?></td>
...
<?php
    ...
    if ((!isset($HTTP_GET_VARS['oID']) || (isset($HTTP_GET_VARS['oID']) && ($HTTP_GET_VARS['oID'] == $orders['orders_id']))) && !isset($oInfo)) {
      ...
      // find out order type
      $sql = "select products_id, rp_profile_id from " . TABLE_ORDERS_PRODUCTS . " WHERE orders_id = " . $orders['orders_id'];
      $rs = tep_db_query($sql);
      $typeArr = array();
      while($tA = tep_db_fetch_array($rs)){
          if(strlen($tA['rp_profile_id'])>0){
            $typeArr[] = 'recurring';
          }else{
            $typeArr = array('standard');
          }
      }
...
?>
                ...
                <td class="dataTableContent" align="right"><?php echo $typeArr[0]; ?></td>
