...
// TABLES: tax_rates
  ...
  function tep_get_paypal_dp_country(){
      $sql = 'select configuration_value from ' . TABLE_CONFIGURATION . ' where configuration_key = \'MODULE_PAYMENT_PAYPAL_DIRECT_COUNTRY\'';
      $rs = tep_db_query($sql);
  ...
  function tep_cfg_pull_down_paypal_dp_country($iso2, $key){
      $name = (($key) ? 'configuration[' . $key . ']' : 'configuration_value');
      $coutryArr = array('US', 'UK', 'CA');
      foreach ($coutryArr as $country){
          $paArr[] = array('id' =>  $country,
                            'text'  =>  $country);
      }
      return tep_draw_pull_down_menu($name, $paArr, $iso2);
  }
