...
// products price
// rp edit for product type
        $product_query = tep_db_query("select products_id, products_price, products_tax_class_id, products_weight, products_type from " . TABLE_PRODUCTS . " where products_id='" . (int)tep_get_prid($products_id) . "'");
        if ($product = tep_db_fetch_array($product_query)) {
          ...
          // rp trial ? or rp product at all
          $rp_trial = false;
          if($product['products_type'] == 'recurring'){
            $rp_product_query = tep_db_query('SELECT billingPeriod, billingFrequency, totalBillingCycles, trialBillingPeriod, trialBillingFrequency, trialTotalBillingCycles, trialAmt FROM ' . TABLE_RP_PAYPAL_PRODUCT_PROFILE . ' WHERE products_id=' . (int)tep_get_prid($products_id));
          ...
          // trial tax - (initial amount will only bill for the trail tax amount if taxable)
          if(!$rp_trial){
            $this->total += tep_add_tax($products_price, $products_tax) * $qty;
          }else{
            $this->total += tep_add_tax($rp_product['trialAmt'], $products_tax) * $qty;
          }
...
// attributes price
    ...
    function get_products() {
      ...
      while (list($products_id, ) = each($this->contents)) {
          // added products_type
        $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_model, p.products_price, p.products_weight, p.products_tax_class_id, p.products_type from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id='" . (int)tep_get_prid($products_id) . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
        if ($products = tep_db_fetch_array($products_query)) {
          ...
          // rp product attiribute
          $rp_array = array();
          $rp_product_query = tep_db_query('SELECT billingPeriod, billingFrequency, totalBillingCycles, trialBillingPeriod, trialBillingFrequency, trialTotalBillingCycles, trialAmt FROM ' . TABLE_RP_PAYPAL_PRODUCT_PROFILE . ' WHERE products_id=' . (int)tep_get_prid($products_id));
          if($rp_product = tep_db_fetch_array($rp_product_query)){
              $rp_array['amt'] = $products_price;
              $rp_array = $rp_product;
            if(strlen($rp_product['trialBillingPeriod'])>0){
                // reset the product price, as initial price will be the trial price
                $products_price = $rp_product['trialAmt'];
            }
          }
