...
*/
  ...
  $action = (isset($HTTP_GET_VARS['action']) ? $HTTP_GET_VARS['action'] : '');
    ...
    // see if we have product types to worry about (standard, recurring)
    // set payment module dir
    $rp_enabled = false;
    $payment_module_directory = DIR_FS_CATALOG_MODULES . 'payment/';
    $file_extension = substr($PHP_SELF, strrpos($PHP_SELF, '.'));
    $directory_array = array();
    if ($dir = @dir($payment_module_directory)) {
        while ($file = $dir->read()) {
            if (!is_dir($payment_module_directory . $file)) {
    ...
    foreach ($directory_array as $require_class_file){
        require_once(DIR_FS_CATALOG_LANGUAGES . $language . '/modules/payment/' . $require_class_file);
        require_once($payment_module_directory . $require_class_file);
        $class = substr($require_class_file, 0, strrpos($require_class_file, '.'));
        if (tep_class_exists($class)) {
            $module = new $class;
            if(isset($module->rp_enabled)){
                if($module->rp_enabled){
                    $rp_enabled = true;
                }
            }
        }
      ...
      case 'update_product':
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          $sql_data_array = array('products_quantity' => (int)tep_db_prepare_input($HTTP_POST_VARS['products_quantity']),
                                  ...
                                  'products_type'   => tep_db_prepare_input($HTTP_POST_VARS['product_type']));
          ...
          if ($action == 'insert_product') {
            ...
            if($HTTP_POST_VARS['product_type'] == 'recurring'){
                // rp insert for rp profile data
                $trial = false;
                if(isset($HTTP_POST_VARS['rpTrialPeriod'])){
                    if($HTTP_POST_VARS['rpTrialPeriod'] == 'on'){
                        $trial = true;
                    }
                }
                if($trial){
                    $sql_data_array = array('products_id'               => $products_id,
                                            'profileStartDate'          => $HTTP_POST_VARS['RP_start_date'],
                                            'billingPeriod'             => $HTTP_POST_VARS['rpBillingPeriod'],
                                            'billingFrequency'          => $HTTP_POST_VARS['rpBillingFrequency'],
                                            'totalBillingCycles'        => $HTTP_POST_VARS['rpNumberPayments'],
                                            // 'amt'                       => 0,  // this amount is stored with the product
                                            'trialProfileStartDate'     => $HTTP_POST_VARS['RP_start_date'],
                                            'trialBillingPeriod'        => $HTTP_POST_VARS['rpTrialBillingPeriod'],
                                            'trialBillingFrequency'     => $HTTP_POST_VARS['rpTrialBillingFrequency'],
                                            'trialTotalBillingCycles'   => $HTTP_POST_VARS['rpTrialNumberPayments'],
                                            'trialAmt'                  => $HTTP_POST_VARS['rpTrialPrice']
                    );
                }else{
                    $sql_data_array = array('products_id'               => $products_id,
                                            'profileStartDate'          => $HTTP_POST_VARS['RP_start_date'],
                ...
                if(isset($HTTP_POST_VARS['rpOneTimeTransaction'])){
                    if($HTTP_POST_VARS['rpOneTimeTransaction'] == '1'){
                        $sql_data_array['initAmt']                      = $HTTP_POST_VARS['rpIntAmt'];
                    }
                }
                if(isset($HTTP_POST_VARS['rpCreateProfileTransactionFailure'])){
                    if($HTTP_POST_VARS['rpCreateProfileTransactionFailure'] == '1'){
                        $sql_data_array['failedInitAmtAction']          = 'ContinueOnFailure';
                    }else{
                        $sql_data_array['failedInitAmtAction']          = 'CancelOnFailure';
                    }
                }else{
                    $sql_data_array['failedInitAmtAction']          = 'CancelOnFailure';
                }
...
<?php
  ...
  if ($action == 'new_product') {
    ...
    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id, p.products_type from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
      // rp - add in the rp data
      if($product['products_type'] == 'recurring'){
          $rp_rs = tep_db_query('SELECT * FROM ' . TABLE_RP_PAYPAL_PRODUCT_PROFILE . ' WHERE products_id = ' . $product['products_id']);
          if($row = tep_db_fetch_array($rp_rs)){
            $product = array_merge((array)$product, (array)$row);
          }
      }
...
?>
<body marginwidth="0" marginheight="0" topmargin="0" bottommargin="0" leftmargin="0" rightmargin="0" bgcolor="#FFFFFF" onload="SetFocus(); SetAllowedBillingFrequencies('rpBilling', '<?php print $pInfo->billingFrequency;?>'); SetAllowedBillingFrequencies('rpTrialBilling', '<?php print trim($pInfo->trialBillingFrequency);?>');">
<div id="spiffycalendar" class="text"></div>
<!-- header //-->
<?php require(DIR_WS_INCLUDES . 'header.php'); ?>
<!-- header_eof //-->
...
//--></script>
<script language="javascript"><!--
var dateAvailableRP = new ctlSpiffyCalendarBox("dateAvailableRP", "new_product", "RP_start_date","btnDate2","",scBTNMODE_CUSTOMBLUE);
//--></script>
<script language="javascript"><!--
...
function updateGross() {
  ...
  var grossValue = document.forms["new_product"].products_price.value;
    document.getElementById('rpPrice').value =  document.getElementById('products_price').value;
  if (taxRate > 0) {
...
function updateProductNet() {
    var txt = document.getElementById('rpPrice').value;
    document.getElementById('products_price').value = txt;
...
?>
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('product_type', array(array('id' => 'standard', 'text' => 'Standard'),array('id' => 'recurring', 'text' => 'Recurring')), $pInfo->products_type,'id=\'product_type\' onchange="showFromSelectRP(\'rpProduct\', \'product_type\');"'); ?></td>
          </tr>
            <tr><td colspan="2">
            <?php
          ...
          <tr bgcolor="#ebebff">
            <td colspan="2">Recurring Payment Values<?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            <td class="main">Recurring payments start date<br><small>(YYYY-MM-DD)</small></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;'; ?><script language="javascript">dateAvailableRP.writeControl(); dateAvailableRP.dateFormat="yyyy-MM-dd";</script></td>
          </tr>
          <tr>
            <td class="main" align="right">
            <?php
            if(strlen($pInfo->trialBillingPeriod)>0){
            ?>
          ...
          <tr>
            ...
            ?>
            <TABLE width="97%" align="center" class="trialrptable" cellpadding="2">
                <tr>
                    <td colspan="2"><B>Trial Period</B></td>
                </tr>
                <tr>
                    <td>Trial payments cycle</td>
                    <td><?php print tep_draw_separator('pixel_trans.gif', '36', '15') . '&nbsp;' .tep_draw_pull_down_menu('rpTrialBillingPeriod', array(array('id' => 'day', 'text' => 'Day'),array('id' => 'week', 'text' => 'Week'),array('id' => 'semimonth', 'text' => 'SemiMonth'),array('id' => 'month', 'text' => 'Month'),array('id' => 'year', 'text' => 'Year')), $pInfo->trialBillingPeriod, ' id="rpTrialBillingPeriod" onchange="SetAllowedBillingFrequencies(\'rpTrialBilling\', \'\');"'). '&nbsp;cycle frequency&nbsp;';?><select id="rpTrialBillingFrequency" name="rpTrialBillingFrequency" /></td>
                </tr>
                <tr>
                    <td>Number of Recurring Payments</td><INPUT type="hidden" name="rpTrialNumberPaymentsOption" value="true">
                    <td><?php print tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .tep_draw_input_field('rpTrialNumberPayments', $pInfo->trialTotalBillingCycles, '');?></td>
                </tr>

                <tr>
                    <td>Net Amount</td>
                    <td><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('rpTrialPrice', $pInfo->trialAmt, ''); ?></td>
                </tr>
                ...
                <tr>
                    <td colspan="2"><B>Recurring Payments</B></td>
                </tr>
                <tr>
                    <td>Payments cycle</td>
                    <td><?php print tep_draw_separator('pixel_trans.gif', '36', '15') . '&nbsp;' .tep_draw_pull_down_menu('rpBillingPeriod', array(array('id' => 'day', 'text' => 'Day'),array('id' => 'week', 'text' => 'Week'),array('id' => 'semimonth', 'text' => 'SemiMonth'),array('id' => 'month', 'text' => 'Month'),array('id' => 'year', 'text' => 'Year')), $pInfo->billingPeriod, ' id="rpBillingPeriod" onchange="SetAllowedBillingFrequencies(\'rpBilling\', \'\');"'). '&nbsp;cycle frequency&nbsp;';?><select id="rpBillingFrequency" name="rpBillingFrequency" /></td>
                </tr>
                <tr>
                    <td>Number of Recurring Payments</td>
                    <td><?php print tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('rpNumberPaymentsOption', 'enddate', true) .  tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .tep_draw_input_field('rpNumberPayments', $pInfo->totalBillingCycles, '');?></td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                    <td><?php print tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('rpNumberPaymentsOption', 'noenddate', false) .  tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;';?>No end date <small>(Payments will continue until you suspend them)</small></td>
                </tr>
                <tr>
                    <td>Net Amount</td>
                    <td><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('rpPrice', '', ' onchange="updateProductNet();" id="rpPrice"'); ?></td>
                </tr>
                <tr>
                    <td colspan="2"><B>One-time transaction</B></td>
                </tr>
                <tr>
                    <td title="Select one-time transactions for charges like setup fees. You can only create them when you create a profile and you won't be able to edit or reactivate your one-time transactions later.
...
?>&nbsp;Process one-time transaction  &nbsp;</td>
          ...
          <tr bgcolor="#ebebff">
            ...
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_price', $pInfo->products_price, 'onKeyUp="updateGross()" id="products_price"'); ?></td>
          </tr>
          <tr bgcolor="#ebebff">
...
<?php
      ...
      if ($pInfo->product_type == 'recurring'){
?>
    <!-- recurring here -->
<?php
      }
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search'])) {
      ...
      $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_type from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$current_category_id . "' order by pd.products_name");
    }
...
// find out the rating average from customer reviews
        ...
        if(isset($pInfo) && is_object($pInfo)){
            if($pInfo->products_type == 'recurring'){
                $productQuery = tep_db_query('SELECT * FROM ' . TABLE_RP_PAYPAL_PRODUCT_PROFILE . ' WHERE products_id = ' . $pInfo->products_id);
                $rpArrRs = tep_db_fetch_array($productQuery);
                $pInfo_array = array_merge((array)$pInfo_array, (array)$rpArrRs);
                $pInfo = new objectInfo($pInfo_array);
            }
        }
      }
      ...
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            $contents[] = array('text' => '<br>' . TEXT_PRODUCTS_PRICE_INFO . ' ' . $currencies->format($pInfo->products_price) . '<br>' . TEXT_PRODUCTS_QUANTITY_INFO . ' ' . $pInfo->products_quantity . '<br>Product Type: '.ucfirst($pInfo->products_type));
