...
if(isset($_GET['action'])){
	switch($_GET['action']) {
		...
		case "start_container":
			$outputShell = shell_exec("sudo /usr/sbin/vzctl start $ctid");
		break;
		case "stop_container":
			$outputShell = shell_exec("sudo /usr/sbin/vzctl stop $ctid");
		break;
		case "restart_container":
			$outputShell = shell_exec("sudo /usr/sbin/vzctl restart $ctid");
		break;	
		// Default Break
		default:
		break;
...
$ctid_status = shell_exec("sudo /usr/sbin/vzctl status $ctid |cut -d\  -f5");
?>
...
<body>
	...
	<!--Monitors -->
		...
		<div class="in">
<strong>Hostname:</strong> <pre><?php echo shell_exec("sudo /usr/sbin/vzctl exec $ctid hostname"); ?></pre><br />
<strong>Image:</strong> <pre><?php echo shell_exec('sudo grep OSTEMPLATE /etc/vz/conf/999.conf |awk -F\" {\'print $2\'}'); ?></pre><br />
<strong>Uptime and Load:</strong><pre><?php echo shell_exec("sudo /usr/sbin/vzctl exec $ctid uptime"); ?></pre><br />
<strong>Kernel Version:</strong> <pre><?php echo shell_exec("sudo /usr/sbin/vzctl exec $ctid uname -srm"); ?></pre><br />
<strong>Memory Usage:</strong> <pre><?php echo shell_exec("sudo /usr/sbin/vzctl exec $ctid free -m"); ?></pre><br />
<strong>Disk Usage:</strong> <pre><?php echo shell_exec("sudo /usr/sbin/vzctl exec $ctid df -h"); ?></pre></br >
