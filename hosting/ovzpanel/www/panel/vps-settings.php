...
if(isset($_POST['changehostname'])){
	$newhostnamesecure = preg_replace('/[^A-Za-z0-9.]/', '', $_POST['n_hostname']);
		$outputShell = shell_exec("sudo /usr/sbin/vzctl set $ctid --hostname $newhostnamesecure --save");		
}

if(isset($_POST['changerootpassword'])){
    $newpasswordsecure = preg_replace('/\s+/', '', $_POST['n_password']);
        $outputShell = shell_exec("sudo /usr/sbin/vzctl set $ctid --userpasswd root:$newpasswordsecure --save");
}
